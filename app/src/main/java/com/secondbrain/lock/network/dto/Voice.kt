 ...
 * "play_briefing" = "play my briefing" / "what's my daily briefing" — no row created, so [title]/
 * [para]/[paraConfidence] default to blank rather than being required for that type. */
@Serializable
 ...
 * [classification]'s `type` (a `tasks` row vs a `notes` row) rather than being one fixed DTO — and
 * is absent/null entirely for the `play_briefing` type, which creates no row at all.
 */
@Serializable
